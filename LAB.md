...
Выполнил `git switch --detach HEAD~3`, Git переключил HEAD прямо на коммит, а не на ветку. Проверил через `git status` и `git branch` - увидели `HEAD detached at 8f0f4a9`.

Создал файл `experiment.md` с текстом "Экспериментальная правка: проверяю гипотезу", добавил и закоммитил: `git commit -m "exp: проверка гипотезы про X"`. Коммит получил хеш, но ни одна ветка на него не указывала.

Не переключаясь на main, выполнил `git switch -c experiment/saved-from-detached`. Эта команда создала новую ветку, указывающую на текущий (detached) коммит и переключила HEAD на неё. Теперь коммит стал частью ветки и не потеряется. Затем запушл ветку.

Во второй части специально "потерял" коммит: сделал его в detached HEAD, переключился на main без сохранения. Git предупредил, что коммит остался не привязан к веткам. Через git reflog нашел запись HEAD@{1}: commit: exp: ещё одна гипотеза с хешем 3b8d567 этого коммита. Используя хеш, выполнил `git switch -c experiment/recovered-by-reflog 3b8d567`  — создал ветку и вернул "потерянный" коммит в историю. Reflog хранит локальную историю перемещений HEAD (включая коммиты, от которых отказались), поэтому позволяет восстановить коммиты, даже если на них нет веток.
